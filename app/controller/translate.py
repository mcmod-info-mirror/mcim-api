...
async def modrinth_translate_batch(
    ...
    results: List[ModrinthTranslation] = await request.app.state.aio_mongo_engine.find(
        ModrinthTranslation, query.in_(ModrinthTranslation.project_id, project_ids.project_ids)
    )
...
async def curseforge_translate_batch(
    ...
    results: List[CurseForgeTranslation] = await request.app.state.aio_mongo_engine.find(
